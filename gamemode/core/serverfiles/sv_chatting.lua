...
function GM:PlayerCanSeePlayersChat(strText, bTeamOnly, pListener, pSpeaker)
  ...
  if tblText[#tblText] == "!" and #tblText > 1 then intChatDistance = 7000 end
  if tblText[#tblText] == "@" and #tblText > 1 then intChatDistance = 7000  if not pListener:IsInSquad(pSpeaker) or not pSpeaker:IsInSquad(pListener) then return false end end
  if pListener:GetPos():Distance(pSpeaker:GetPos()) >= intChatDistance then return false end --Too Far
  return true --All good :)
end
